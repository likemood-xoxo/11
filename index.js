...
function renderCard(canvas, text, charName) {
    ...
    let H = isLand ? 720 : 960;

    const PAD = 64;
    const TOP_FIXED = 110;
    const BOTTOM_FIXED = 90;
    ...
    if (manualFs >= 8) {
        ...
        for(let f = manualFs; f >= minFs; f--) {

            ctx.font = `${f}px ${font}`;

            const l = Math.round(f * 1.72);
            const g = Math.round(f);

            let tot = 0;

            paras.forEach((p,i)=>{
                tot += wrapText(ctx,p,W-PAD*2).length * l;
                if(i < paras.length-1) tot += g;
            });

           if(tot <= H-TOP_FIXED-BOTTOM_FIXED){
                fs = f;
                lh = l;
                pg = g;
                break;
    ...
    const previewLines = paras.map(p => wrapText(ctx, p, W - PAD * 2));

    let neededH = 0;

    previewLines.forEach((lines,i)=>{
        neededH += lines.length * lh;

        if(i < previewLines.length - 1){
            neededH += pg;
        }
    });
    ...
    const minH = isLand ? 720 : 960;


    // 긴 글이면 캔버스 높이 확장
    const contentArea = minH - TOP_FIXED - BOTTOM_FIXED;

if (neededH > contentArea) {
    H = TOP_FIXED + neededH + BOTTOM_FIXED;
} else {
    H = minH;
}
...
ctx.font = bodyFont;
    ...
    if (bg==='unsplash' && photoUrl) {
        ...
        img.onload=()=>{
            ...
            drawContent(ctx,W,H,PAD,lh,pg,paras,charName,bodyFont,metaFont,tc,align,{rule:'rgba(255,255,255,0.4)',ornament:'rgba(255,255,255,0.7)'},TOP_FIXED,BOTTOM_FIXED);
        ...
        img.onerror=()=>{ ctx.fillStyle='#1a1714'; ctx.fillRect(0,0,W,H); drawContent(ctx,W,H,PAD,lh,pg,paras,charName,bodyFont,metaFont,tc,align,THEMES.dark,TOP_FIXED,BOTTOM_FIXED); };
    ...
    drawContent(ctx,W,H,PAD,lh,pg,paras,charName,bodyFont,metaFont,tc,align,theme,TOP_FIXED,BOTTOM_FIXED);
}

function drawContent(ctx,W,H,PAD,lh,pg,paras,charName,bodyFont,metaFont,tc,align,theme,TOP_FIXED,BOTTOM_FIXED) {
